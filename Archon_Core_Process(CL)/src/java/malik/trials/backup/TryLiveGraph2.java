...
public class TryLiveGraph2{
	...
	private static String TABLESTART = "<TABLE BORDER='0' CELLBORDER='1' CELLSPACING='0' CELLPADDING='4'>";
	private static String THSTART = "<TH>";
	private static String TRSTART = "<TR>";
	private static String SEARCHKEY = ":::COLNAME:::";
	private static String TDSTART = "<TD PORT='"+SEARCHKEY+"'>";
	private static String THDSTART = "<TD PORT='"+SEARCHKEY+"' BGCOLOR='GRAY'>";
	private static String TDEND = "</TD>";
	private static String TREND = "</TR>";
	private static String THEND = "</TH>";
	...
	private static void graphGenerator() {
		for (String table: colmap.keySet()){
			StringBuffer sb = new StringBuffer();
			sb.append(TABLESTART);
			sb.append(THSTART).append(value(THDSTART,table)).append(table).append(TDEND).append(THEND);
			for (int pos : colmap.get(table).keySet()) {
				if(colmap.get(table).get(pos).isPartOfPrimaryKey())
					sb.append(TRSTART).append(value(TDSTART,colmap.get(table).get(pos).getName())).append(BOLDSTART).append(colmap.get(table).get(pos).getName()).append(BOLDEND).append(TDEND).append(TREND);
				else	
					sb.append(TRSTART).append(value(TDSTART,colmap.get(table).get(pos).getName())).append(colmap.get(table).get(pos).getName()).append(TDEND).append(TREND);
			}	
			sb.append(TABLEEND);
	...
	private static String value(String srcString, String valueString) {
		return srcString.replace(SEARCHKEY, valueString);
	}
