...
public final class CatalogWithCounts extends BaseCatalogDecorator {
	...
	public CatalogWithCounts(final Catalog catalog, final Connection connection, final SchemaCrawlerOptions options)
		...
		for (final Table table : allTables) {
			...
			try {
				...
				final Query query_big = Operation.count_Big.getQuery();

				try {
					final long count = executeForLong(query_big, connection, table);
					counts.put(table, count);
					addRowCountToTable(table, count);
				} catch (final SQLException | SchemaCrawlerException e1) {
					LOGGER.log(Level.WARNING, e1, new StringFormat("Could not get count for, %s", table));
				}
			}
		}

		reduce(Table.class, new TablesReducer(options, new TableCountFilter(options)));
	}
