...
public class OutputOptions implements Options {
	...
	private boolean moveFileToNas = false;
	private boolean setIndex = false;
	private String blobRefAttribute;
	...
	private LinkedHashMap<String, Object[]> datatype = new LinkedHashMap<String, Object[]>();
	...
	public OutputOptions(final Config config) {
		...
		setInputEncoding(configProperties.getStringValue(SC_INPUT_ENCODING, StandardCharsets.UTF_8.name()));
		setOutputEncoding(configProperties.getStringValue(SC_OUTPUT_ENCODING, StandardCharsets.UTF_8.name()));
	}
	...
	public OutputOptions(final OutputFormat outputFormat, final Path outputFile) {
		this(requireNonNull(outputFormat, "No output format provided").getFormat(), outputFile);
	}
	...
	public OutputOptions(final OutputFormat outputFormat, final Writer writer) {
		this(requireNonNull(outputFormat, "No output format provided").getFormat(), writer);
	}
	 ...
	 */
	public void setCompressedInputFile(final Path inputFile) throws IOException {
	...
	/**
	 * Sets the name of the output file for compressed output. It is important
	 * to note that the output encoding should be available at this point.
	 *
	 * @param outputFileName
	 *            Output file name.
	 * @throws IOException
	 */
	public void setCompressedOutputFile(final Path outputFile) throws IOException {
	...
	public String getBlobRefAttribute() {
		return blobRefAttribute;
	}

	public void setBlobRefAttribute(String blobRefAttribute) {
		this.blobRefAttribute = blobRefAttribute;
	}

	public boolean isMoveFileToNas() {
		return moveFileToNas;
	}
	...
	public void setMoveFileToNas(boolean moveFileToNas) {
		this.moveFileToNas = moveFileToNas;
	}
	 ...
	 *             When file cannot be read
	 */
	public void setInputFile(final Path inputFile) throws IOException {
		requireNonNull(inputFile, "No input file provided");
	...
	/**
	 * Sets the name of the input resource, first from a file, failing which
	 * from the classpath. It is important to note that the input encoding
	 ...
	 */
	public void setInputResourceName(final String inputResourceName) throws IOException {
	...
	public void setOutputFormatValue(final String outputFormatValue) {
		this.outputFormatValue = requireNonNull(outputFormatValue, "Cannot use null value in a setter");
	}
	...
	public void setSplitDate(boolean splitDate) {
		this.splitDate = splitDate;
	}

	public Path getArReport() {
		return arReport;
	}

	public void setArReport(Path arReport) {
