...
final class SchemaJsonFormatter extends BaseJsonFormatter<SchemaTextOptions> implements SchemaTraversalHandler {
	 ...
	 *             On an exception
	 */
	SchemaJsonFormatter(final SchemaTextDetailType schemaTextDetailType, final SchemaTextOptions options,
	...
	public void handle(final ColumnDataType columnDataType) throws SchemaCrawlerException {
		if (printVerboseDatabaseInfo && isVerbose) {
			try {
				...
				LOGGER.log(Level.FINER, e, new StringFormat("Error outputting ColumnDataType: %s", e.getMessage()));
			}
		}
	}
	...
	public void handle(final Routine routine) {
		try {
			...
			LOGGER.log(Level.FINER, e, new StringFormat("Error outputting Routine: %s", e.getMessage()));
		}
	...
	public void handle(final Sequence sequence) {
		try {
			...
			LOGGER.log(Level.FINER, e, new StringFormat("Error outputting sequence: %s", e.getMessage()));
		}
	...
	public void handle(final Synonym synonym) {
		try {
			...
			LOGGER.log(Level.FINER, e, new StringFormat("Error outputting synonym: %s", e.getMessage()));
		}
		...
		try {
			...
			if (!isBrief) {
				if (isVerbose) {
					...
					final List<WeakAssociationForeignKey> weakAssociations = new ArrayList<>(
							weakAssociationsCollection);
					Collections.sort(weakAssociations);

					if (!options.isHideWeakAssociations()) {
						jsonTable.put("weakAssociations", handleWeakAssociations(weakAssociations));
					}
				}
			...
			LOGGER.log(Level.FINER, e, new StringFormat("Error outputting table: %s", e.getMessage()));
		}
	}
	...
	private JSONArray handleColumnReferences(final BaseForeignKey<? extends ColumnReference> foreignKey) {
		...
		for (final ColumnReference columnReference : foreignKey) {
			try {
				...
				LOGGER.log(Level.FINER, e, new StringFormat("Error outputting column reference: %s", e.getMessage()));
			}
		}
		...
		for (final ForeignKey foreignKey : foreignKeys) {
			if (foreignKey != null) {
				try {
					...
					LOGGER.log(Level.FINER, e, new StringFormat("Error outputting foreign key: %s", e.getMessage()));
				}
			}
		}
	...
	private JSONObject handleIndex(final Index index) {
		...
		try {
			...
			LOGGER.log(Level.FINER, e, new StringFormat("Error outputting index: %s", e.getMessage()));
		}
	...
	private JSONObject handleRoutineColumn(final RoutineColumn<?> column) {
		...
		try {
			...
			LOGGER.log(Level.FINER, e, new StringFormat("Error outputting routine column: %s", e.getMessage()));
		}
	...
	private JSONObject handleTableColumn(final Column column) {
		...
		try {
			...
			LOGGER.log(Level.FINER, e, new StringFormat("Error outputting column: %s", e.getMessage()));
		}
	...
	private JSONObject handleTableConstraint(final TableConstraint constraint) {
		...
		try {
			...
			for (final TableConstraintColumn tableConstraintColumn : constraint.getColumns()) {
				jsonConstraint.accumulate("columns", handleTableColumn(tableConstraintColumn));
			}
			printDefinition(constraint, jsonConstraint);
		} catch (final JSONException e) {
			LOGGER.log(Level.FINER, e, new StringFormat("Error outputting table constraint: %s", e.getMessage()));
		}
		...
		for (final Trigger trigger : triggers) {
			if (trigger != null) {
				try {
					...
					LOGGER.log(Level.FINER, e, new StringFormat("Error outputting trigger: %s", e.getMessage()));
				}
			}
		}
		return jsonTriggers;
		...
		for (final WeakAssociationForeignKey weakFk : weakAssociations) {
			if (weakFk != null) {
				try {
					...
					LOGGER.log(Level.FINER, e, new StringFormat("Error outputting weak association: ", e.getMessage()));
				}
			}
		}
	...
	private void printDefinition(final DefinedObject definedObject, final JSONObject jsonObject) throws JSONException {
		if (!isVerbose) {
			return;
		}
	...
	private void printRemarks(final DatabaseObject object, final JSONObject jsonObject) throws JSONException {
	...
	private void printTableRowCount(final Table table, final JSONObject jsonObject) throws JSONException {
