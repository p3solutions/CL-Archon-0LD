...
final class DataTextFormatter extends BaseTabularFormatter<OperationOptions> implements DataTraversalHandler {
	 ...
	 *            Options for text formatting of data
	 */
	DataTextFormatter(final Operation operation, final OperationOptions options, final OutputOptions outputOptions)
	...
	private void printHeader() {
		if (operation != null) {
			formattingHelper.writeHeader(DocumentHeaderType.subTitle, operation.getDescription());
		} else {
		...
		if (operation == Operation.count) {
			formattingHelper.writeObjectStart();
			formattingHelper.writeObjectNameRow("", operation.getDescription(), "", Color.white);
		}
	}
}
