...
import schemacrawler.tools.text.utility.MetadataXMLFormattingHelper;
...
public abstract class BaseFormatter<O extends BaseTextOptions> implements TraversalHandler {
	...
	protected BaseFormatter(final O options, final boolean printVerboseDatabaseInfo, final OutputOptions outputOptions)
		...
		case xml:
			...
			break;
		case arxml:
			// XMLFormattingHelper.setArMap(new TreeMap<String, TreeMap<String,
			// TreeSet<Integer>>>());
			formattingHelper = new XMLFormattingHelper(out, outputFormat, outputOptions.getOutputFile().toString());
			break;
		case metadataxml:
			formattingHelper = new MetadataXMLFormattingHelper(out, outputFormat);
			break;
		case text:
		default:
	...
	protected String columnNullable(final String columnTypeName, final boolean isNullable) {
		return isNullable ? "" : isLowerCase(columnTypeName) ? " not null" : " NOT NULL";
	}
	...
	protected boolean isColumnSignificant(final Column column) {
		return column != null && (column instanceof IndexColumn || column.isPartOfPrimaryKey()
