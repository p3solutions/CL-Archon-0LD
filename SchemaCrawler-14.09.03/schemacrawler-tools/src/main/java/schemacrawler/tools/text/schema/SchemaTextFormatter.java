...
final class SchemaTextFormatter extends BaseTabularFormatter<SchemaTextOptions> implements SchemaTraversalHandler {
	 ...
	 *             On an exception
	 */
	SchemaTextFormatter(final SchemaTextDetailType schemaTextDetailType, final SchemaTextOptions options,
	...
	public void handle(final Routine routine) {
		...
		formattingHelper.println();
		formattingHelper.writeObjectStart();
		formattingHelper.writeObjectNameRow(nodeId(routine), routineName, routineType, colorMap.getColor(routine));
	...
	public void handle(final Sequence sequence) {
		...
		formattingHelper.println();
		formattingHelper.writeObjectStart();
		formattingHelper.writeObjectNameRow(nodeId(sequence), sequenceName, sequenceType, colorMap.getColor(sequence));
		printRemarks(sequence);

		if (!isBrief) {
			formattingHelper.writeDetailRow("", "increment", String.valueOf(sequence.getIncrement()));
			formattingHelper.writeDetailRow("", "minimum value", String.valueOf(sequence.getMinimumValue()));
			formattingHelper.writeDetailRow("", "maximum value", String.valueOf(sequence.getMaximumValue()));
	...
	public void handle(final Synonym synonym) {
		...
		formattingHelper.println();
		formattingHelper.writeObjectStart();
		formattingHelper.writeObjectNameRow(nodeId(synonym), synonymName, synonymType, colorMap.getColor(synonym));
		...
		if (!isBrief) {
			...
			formattingHelper.writeDetailRow("", String.format("%s %s %s", synonym.getName(),
					formattingHelper.createRightArrow(), referencedObjectName), "");
		}
	...
	public void handle(final Table table) {
		...
		formattingHelper.println();
		formattingHelper.writeObjectStart();
		formattingHelper.writeObjectNameRow(nodeId(table), tableName, tableType, colorMap.getColor(table));
		printRemarks(table);
	...
	private void printColumnDataType(final ColumnDataType columnDataType) {
		...
		final String userDefined = negate(columnDataType.isUserDefined(), "user defined");
		final String nullable = negate(columnDataType.isNullable(), "nullable");
		final String autoIncrementable = negate(columnDataType.isAutoIncrementable(), "auto-incrementable");
	...
	private void printForeignKeys(final Table table) {
		...
		for (final ForeignKey foreignKey : foreignKeys) {
			if (foreignKey != null) {
				...
				if (updateRule == deleteRule && updateRule != ForeignKeyUpdateRule.unknown) {
	...
	private void printIndexes(final Collection<Index> indexesCollection) {
		...
		for (final Index index : indexes) {
			if (index != null) {
				...
				final String indexDetails = "[" + (index.isUnique() ? "" : "non-") + "unique " + indexTypeString
						+ "index]";
		...
		for (final Privilege<Table> privilege : privileges) {
			if (privilege != null) {
				...
				for (final Grant<?> grant : privilege.getGrants()) {
					final String grantedFrom = String.format("%s %s %s%s", grant.getGrantor(),
	...
	private void printRoutineColumns(final List<? extends RoutineColumn<?>> columns) {
		...
		for (final RoutineColumn<?> column : columns) {
			...
			formattingHelper.writeDetailRow(ordinalNumberString, column.getName(), columnType.toString());
		}
	}
	...
	private void printTableColumns(final List<? extends Column> columns, final boolean showHidden) {
		if (columns.isEmpty()) {
			return;
		}
		...
		for (final Column column : columns) {
			...
			formattingHelper.writeDetailRow(ordinalNumberString, columnName, true, columnDetails, emphasize);
	...
	private void printTableConstraints(final Collection<TableConstraint> constraintsCollection) {
		...
		final List<TableConstraint> constraints = constraintsCollection.stream().filter(constraint -> (EnumSet
				.of(TableConstraintType.check, TableConstraintType.unique).contains(constraint.getConstraintType())))
